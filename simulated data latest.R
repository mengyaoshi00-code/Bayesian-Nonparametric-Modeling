...
simulate_aft_weibull <- function(n = 2000, K = 8, seed = 123,
  ...
  ## 2) Covariates L1, L2 isolate

  L1 <- rnorm(n, 0, 1)
  L2 <- 0.6*L1 + rnorm(n, 0, 1)  # add correlation if desired
...
# Revise 11.10
# For each subgroup g, calculates the true risk difference at t =1 year
compute_true_RD1y_mc <- function(dat, t1 = 1.0, B = 5000, seed = 42) {
  set.seed(seed)
  shape <- attr(dat, "shape_event"); tau_g <- attr(dat, "tau_g")
  by_g  <- split(seq_len(nrow(dat)), dat$G)
  
  RD1y <- sapply(names(by_g), function(gg) {
    idx <- by_g[[gg]]; g <- as.integer(gg)
    eta0 <- 1.5 + 0.3*dat$L1[idx] + 0.4*dat$L2[idx] + 0.3*dat$L3[idx]
    mu0  <- exp(eta0 + tau_g[g] * 0)
    mu1  <- exp(eta0 + tau_g[g] * 1)
    scale0 <- mu0 / gamma(1 + 1/shape)
    scale1 <- mu1 / gamma(1 + 1/shape)
    # MC: average P(T<=t1) via simulation
    p0 <- rowMeans(matrix(rweibull(length(idx)*B, shape, rep(scale0, each=B)) <= t1, nrow=length(idx)))
    p1 <- rowMeans(matrix(rweibull(length(idx)*B, shape, rep(scale1, each=B)) <= t1, nrow=length(idx)))
    mean(p1 - p0)
  })
...
rd_table <- compute_true_RD1y_mc(dat, t1 = 1.0)
rd_table
